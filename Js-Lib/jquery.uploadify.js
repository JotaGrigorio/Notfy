...
(function ($) {
        ...
        init: function (options, swfUploadOptions) {
            ...
            return this.each(function () {
                ...
                var settings = $.extend({
                    ...
                    swf: '/js-lib/uploadify.swf',  // The path to the uploadify SWF file
                    uploader: '/Handler/Upload.ashx',  // The path to the server-side upload script
                    ...
                    buttonText: 'Selecione o arquivo',     // The text to use for the browse button
                    checkExisting: false,              // The path to a server-side script that checks for existing files on the server
                    debug: false,              // Turn on swfUpload debugging mode
                    fileObjName: 'Filedata',         // The name of the file object to use in your server-side script
                    fileSizeLimit: 16384,               // The maximum size of an uploadable file in KB (Accepts units B KB MB GB if string, 0 for no limit)
                    fileTypeDesc: 'Arquivo PDF',      // The description for file types in the browse dialog
                    fileTypeExts: '*.pdf',              // Allowed extensions in the browse dialog (server-side validation should also be used)
                    ...
                    width: '240px',                // The width of the browse button - Sempre tem que ser 10px a mais,
                    buttonFloat: "left",
                ...
                if (flashInstalled) {
                    ...
                    // Create the button
                    var $button = $('<div />', {
                        'id': settings.id + 'uploadify-button',
                    ...
                    var widthObjetoFlash;

                    if (settings.width.indexOf('%') == -1)
                        widthObjetoFlash = settings.width;
                    else
                        widthObjetoFlash = (parseInt(settings.width.replace('%', '')) - 3).toString() + '%';

                    $('#' + swfuploadify.movieName).css({
                        'position': 'absolute',
                        'z-index': 1,
                        // Kaique                        
                        //'width': (parseInt(settings.width.replace('%', '')) - 3).toString() + '%'
                        'width': widthObjetoFlash
                    });
                    ...
                    var componenteUp = $($this);


                    var leftUp = componenteUp.offset().left,
                        topUp = componenteUp.offset().top - 30;

                    $("body").append("<div id='duvidaUp" + componenteUp.attr("id") + "' style='cursor: pointer; position: absolute; border: 1px solid #EE3B3B; color: white; padding: 2px; background-color: #EE3B3B; top: " + topUp + "px; left: " + leftUp + "px;'>Falha ao carregar plug-in. Clique aqui.</div><br/>");

                    var componenteMsgErroUp = $("#" + "duvidaUp" + componenteUp.attr("id"));

                    componenteMsgErroUp.click(function() {
                        window.open("/Manuais/Manual.Upload.Desconfigurado.pdf");
                    });
    ...
    var handlers = {
        ...
        onDialogClose: function (filesSelected, filesQueued, queueLength) {
            ...
            if ($.inArray('onDialogClose', settings.overrideEvents) < 0) {
                if (this.queueData.filesErrored > 0) {
                    MsgAlerta(this.queueData.errorMsg);
                }
            }
        ...
        onSelect: function (file) {
            ...
            extFileUpfy = fileName.split('.').pop();
            ...
            if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
                switch (errorCode) {
                    ...
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\nO arquivo "' + file.name + '" excedeu o limite máximo (16 MB).';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\nO arquivo "' + file.name + '" está vazio, favor informar um arquivo válido.';
                        break;
                    case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
                        this.queueData.errorMsg += '\nO arquivo "' + file.name + '" não é um tipo válido de arquivo.';
                        break;
                }
            }
